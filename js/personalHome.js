    ...
    success: function(response) {
      ...
      if (response.statusCode === "200") {
        ...
        $.each(shareList, function(indexInArray, valueOfElement) {
          ...
          var html =
            '<div class="item" shareid="' +
            valueOfElement.id +
            '" >\
          <div class="circle ' +
            getColorCircle(indexInArray % 5) +
            '"></div>\
          <div class="date">\
          <span>' +
            timeObj.month +
        ...
        $.each(likeList, function(indexInArray, valueOfElement) {
          $(
            '#share-content .icon-heart[shareid="' + valueOfElement + '"]'
          ).disHasLike();
        });
      } else {
        if (response.msg !== "没有记录")
          swal({
            title: "获取你的稿件失败",
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            if (response.statusCode === "200") {
              ...
              $('#share-content .item[shareid="' + id + '"]').addClass(
                "animated fadeOut"
              );
              setTimeout(function() {
